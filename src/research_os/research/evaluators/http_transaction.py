"""Deterministic http.transaction evaluator. Records observed HTTP facts, not findings."""
...
class HttpTransactionEvaluator:
    strategy = HTTP_TRANSACTION_EVALUATION_STRATEGY
    version = HTTP_TRANSACTION_EVALUATION_STRATEGY
    ...
    def evaluate(
        ...
        if plan.required_capability != HTTP_TRANSACTION_CAPABILITY:
            raise ResearchInputError("http.transaction evaluator cannot assess other capabilities")
