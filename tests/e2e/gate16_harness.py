"""GATE 16 harness. Drives the existing pipeline. Hidden ground truth stays out."""
...
GATE16_HUMAN = "gate16-human-reviewer"


@dataclass(frozen=True)
class CrossClassProbe:
...
def _plan(origin: str, scenario: SecurityGroundTruthScenario, *, verification: bool = False):
    ...
    resource_id = harness.verification_resource_id if verification else harness.resource_id
    assert actor is not None and resource_id is not None and harness.transition is not None
...
def seed_scenario(uow, scenario: SecurityGroundTruthScenario) -> None:
    prefix = prefix_for(scenario.scenario_id)
    uow.programs.insert(
        ProgramRecord(program_id=f"{prefix}-prog", created_at=NOW, name=f"gate16-{prefix}")
    )
    ...
    uow.hypotheses.insert(
        HypothesisRecord(
            ...
            origin_reference="human-seed-gate16",
            created_at=NOW,
        )
    )
...
def run_scenario(factory, scenario: SecurityGroundTruthScenario) -> ObservedScenarioResult:
    ...
    if harness.in_scope:
        lab = WorkflowLab(harness.fixture_kind)
        origin = lab.start()
...
def run_bola_cross_class(factory) -> ObservedScenarioResult:
    """BOLA probe on the same pipeline. Must remain HTTP_AUTHORIZATION_DIFFERENTIAL."""

    prefix = "xbola"
    run_id = f"{prefix}-run"
    with factory.open() as uow:
        uow.programs.insert(
            ProgramRecord(program_id=f"{prefix}-prog", created_at=NOW, name="gate16-xbola")
        )
        uow.authorization_sources.insert(
            AuthorizationSourceRecord(
                authorization_source_id=f"{prefix}-as",
                program_id=f"{prefix}-prog",
                state="ACTIVE",
                provenance_reference="written-local-lab-auth-xbola",
                created_at=NOW,
            )
        )
        uow.research_runs.insert(
        ...
        uow.hypotheses.insert(
            HypothesisRecord(
                ...
                origin_reference="human-seed-gate16-bola",
                created_at=NOW,
            )
        )
        uow.commit()
    ...
    with GroundTruthLab(TRUE_BOLA) as lab:
        ...
        probe(factory, f"{prefix}-exp", run_id, plan, worker, _allow_scope(), _compiled_scope_for_origin(origin))
        admitted = assess_and_admit(factory, f"{prefix}-exp")
        ...
        with factory.open() as uow:
            ...
            context = CandidateAdmissionContext(
                ...
                authoritative_out_of_scope=False,
                known_duplicate_candidate_id=None,
            )
            workflow_proposal = propose_state_transition_candidate(
                context, proposal_id="must-not-exist"
            )
        proposed = ProposeCandidateFromEvidence(factory, clock=FixedClock()).execute(
            ProposeCandidateFromEvidenceCommand(evidence_id=admitted.evidence_id)
        )
        with factory.open() as uow:
            candidate = uow.candidates.get(proposed.candidate_id) if proposed.candidate_id else None
        ...
        return CrossClassProbe(
            result=ObservedScenarioResult(
            scenario_id="XBOLA_CROSS_CLASS",
            version="1",
            ...
            http_request_count=lab.http_request_count(),
            redirect_followed=False,
            original_experiment_id=f"{prefix}-exp",
            reproduction_experiment_id=None,
            original_request_id=attempts[0].request_id if attempts else None,
            ...
            observed_classification=(
                None if candidate is None else candidate.classification
            ),
            finding_classification=None,
        ),
            workflow_proposal_from_bola_evidence=workflow_proposal,
        )
...
def _snapshot(
    ...
    with factory.open() as uow:
        ...
        observed_classification = None if candidate is None else candidate.classification
        finding_classification = None if not findings else findings[0].classification
